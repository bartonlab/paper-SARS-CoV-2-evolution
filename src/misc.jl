...
"""
    create_variant_dictionary(Reports::Vector{Any}, t::Int64)

Create a dictionary summarizing the sequences and their counts for a given time point.

# Arguments
- `Reports::Vector{Any}`: A vector containing reports of populations at different time points.
- `t::Int64`: The time point of interest for which to gather and process sequences.

# Returns
- `letter_dict::Dict{String, Int64}`: A dictionary where keys are unique sequence identifiers and values are their respective counts.
- `vector_counts::Dict{Vector{Float64}, Int}`: A dictionary mapping each unique sequence (as a vector) to its count.
- `keys_list::Vector{String}`: A list of the sequence identifiers.
- `values_list::Vector{Int64}`: A list of the counts for each sequence identifier.

# Description
This function gathers all the variant sequences at a specific time point `t` from the given population reports. It creates a count dictionary for each unique sequence and assigns names to each sequence based on its length and a series of alphabetic labels. The function returns the labeled dictionary of sequence counts (`letter_dict`), the original sequence-count dictionary (`vector_counts`), and the lists of keys (`keys_list`) and values (`values_list`). The naming convention for sequences is based on the length of each sequence, and unique letters are appended if multiple sequences share the same length.
"""


function create_variant_dictionary(Reports::Vector{Any},t::Int64)

    #gather all sequences at time=t
    seqs=[]
    for s in Reports[Int64(t)]
        push!(seqs,s.variant)
    end 

    #create vector-count dictionary
    vector_counts = Dict{Vector{Float64}, Int}()
    for vec in seqs
    # Update the vector counts dictionary
        vector_counts[vec] = get(vector_counts, vec, 0) + 1
    end

    vector_pairs = collect(vector_counts)
    #Sort the pairs by the length of the vector (first element of each pair)
    sorted_pairs = sort(vector_pairs, by = (pair -> length(pair[1])))
    


    letter_dict = Dict{String, Int64}()
    list_of_sizes=[]
    letters_ = collect('a':'z')
    ...
    for (vec, count) in sorted_pairs
        
        if length(vec) in list_of_sizes
            s_name="s-" * string(length(vec))*"-"*letters_[itr]
            itr+=1
        else
            s_name="s-" * string(length(vec))
            push!(list_of_sizes,length(vec))
            itr=1
        end
    
        letter_dict[s_name] = count 
        push!(values_list,count)
        push!(keys_list,s_name)

    end

    return letter_dict,vector_counts, keys_list, values_list  
...
"""
   export_individual_vectors(individual_vectors::Vector{Any}, filename::String)

Export a list of individuals by serializing and saving it to a file.

# Arguments
- `individual_vectors::Vector{Any}`: A vector containing the individuals to be exported.
- `filename::String`: The name of the file to save the serialized data.

# Description
This function export the reports output of the evolve_pop_EpiModel() function
"""


function export_individual_vectors(individual_vectors::Vector{Any}, filename::String)
    open(filename, "w") do file
        serialize(file, individual_vectors)
    end
end


"""
    import_individuals(filename::String)::Vector{Any}

Import a list of individuals by reading and deserializing data from a file.

# Arguments
- `filename::String`: The name of the file from which to load the serialized data.

# Returns
- `individuals::Vector{Any}`: A vector containing the deserialized list of individuals.

# Description
This function reads serialized data corresponding to reports output of the evolve_pop_EpiModel() function
"""

function import_individual_vectors(filename::String)::Vector{Any}
